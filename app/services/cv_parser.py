...
class CVParser:
    def __init__(self):
        try:
            ...
            logger.warning("spaCy model not found, using basic text parsing")
            self.nlp = None
    ...
    def extract_skills(self, text: str) -> List[str]:
        ...
        # Use spaCy for more advanced skill extraction if available
        if self.use_spacy and self.nlp:
            doc = self.nlp(text)
            for ent in doc.ents:
                if ent.label_ in ['ORG', 'PRODUCT'] and len(ent.text) > 2:
                    skill_candidate = ent.text.strip()
                    if skill_candidate.lower() not in [s.lower() for s in found_skills]:
    ...
    def extract_education(self, text: str) -> List[str]:
        ...
        education_keywords = [
            ...
            'school', 'institute', 'academy', 'certification', 'certificate'
        ]
